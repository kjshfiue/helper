		...
		columns:[[
			...
			{field:'opt',title:'操作',fit:true,formatter:function(val,row,idx){
				var content="<input type='button' value='删除' onClick=\"del('"+row.categorycode+"','"+row.code+"')\" />";
					...
					content+="<input type='button' value='预览' onClick=\"look("+idx+")\"/>";
					return content;
			}},
		]],
		toolbar:"#toolbar",
...
function look(idx){
	...
	$("#myform").find("input[name='categorycode']").val(row.categorycode).readOnly=true;
	...
	$("#myform").find("input[name='isShow']").val(row.isShow).attr("readonly","readonly");
	$("#myform").find("input[name='up_categorycode']").val(row.categorycode);
	$("#myform").find("input[name='up_code']").val(row.code);
	$("#myform").find("input").attr("readonly","readonly")
	$("#add_Dialog").dialog({
		title:"修改字典",
		modal:true,
		close:false
